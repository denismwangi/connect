...
def register():
    ...
    if request.method == "POST" and "img" in request.files:
        ...
        if allowed_file(filename):
            os.mkdir(upload_folder +"/"  + email)
            my_dir = upload_folder +"/"  + email
            pic.save(os.path.join(my_dir, filename))
            image = upload_folder   +"/"  + email + "/" + filename
            namez2 = email +".jpg"
            os.rename(image  , upload_folder   +"/" + email + "/" + email +".jpg")
        ...
        if passc == passc2  and not registered:
            ...
            favs = []
            tags = []
            users.insert_one({"email":email ,'username':username , "password":hashed , "favs" : favs , "tags" : tags })
            #os.rename(image , "imagos.jpg")
...
def profile():
    ...
    tags = acc['tags']
    user = acc['username']
    if os.path.isfile(upload_folder  +"/" + me + "/" + me +".jpg"):
        pass
        profile_pic = upload_folder   +"/" + me + "/" + me +".jpg"
    else:
        profile_pic = "default"
        pass
    profile_pic = upload_folder   +"/" + me + "/" + me +".jpg"
    if request.method == " POST":
        ...
        return render_template('profile.html' , prof = profile_pic ,  me = me , favs = favs , tags = tags)
    return render_template('auth/profile.html' ,   me = me , favs = favs , tags = tags , prof = profile_pic)
